...
Read the surrounding code, not only the diff hunks. A line that looks fine in
isolation is often wrong in context, so check the callers, the types, the error
...
- Each finding: `file:line`, then what is wrong, why it matters, and a concrete
