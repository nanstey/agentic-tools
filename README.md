...
| [`fresh-air`](skills/code-quality/fresh-air/SKILL.md) | Process one smell finding at a time and return a primary (plus optional fallback) refactoring technique with a concrete code-improvement suggestion and confidence, using the shared `_knowledge` mapping layer. |
| [`refactor`](skills/code-quality/refactor/SKILL.md) | Orchestrate the decomposed refactor flow: validate `/sniff` findings, fan out each smell to `fresh-air`, merge recommendations, then hand off to `dev/speclist` for final plan-file authoring. |
| [`sniff`](skills/code-quality/sniff/SKILL.md) | Run a code-review-focused smell checklist backed by the shared `_knowledge` smell dataset (`skills/code-quality/_knowledge/smells/index.json` + per-smell markdown) and return canonical smell mapping with related refactorings for every detected smell. |
