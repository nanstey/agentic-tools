...
4. Return `number,title,body,url,headRefName,headRefOid,baseRefName,baseRefOid,state,isDraft,author,mergeStateStatus`. `baseRefOid`/`headRefOid` are the authoritative PR comparison SHAs (see `changes` for the diff-base protocol that consumes them).
