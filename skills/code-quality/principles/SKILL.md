...
2. Report
   ...
   - Each list item must contain only: status emoji + principle name using the mapping above.
...
3. Details
   ...
   - Order items by outcome severity first (`fail` before `warning`), then by hierarchy importance. Title status emoji + name using the mapping above.
