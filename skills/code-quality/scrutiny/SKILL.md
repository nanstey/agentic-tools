...
name: scrutiny
description: >
  Interrogates a build or design silently across the relevant engineering domains, resolving
  what the codebase and sensible defaults can answer and surfacing only the decisions that
  need the user's authority, then builds and verifies. Use when the user asks to build,
  design, scaffold, architect, or plan any system, feature, service, app, agent, pipeline,
  connector, or tool — especially when the request is short or underspecified — or asks to
  "review this architecture", "what am I missing", "ask me the right questions", or "poke
  holes in this" before implementation.
user-invocable: true
...
> Vendored from [m4vic/socratic](https://github.com/m4vic/socratic) ([MIT](LICENSE), Copyright (c) 2026 m4vic). Changes: skill renamed `socratic` → `scrutiny` (identity and `$scrutiny` triggers), frontmatter adapted to this repo's conventions (outcome-first `description`, `user-invocable`/`disable-model-invocation`). The question bank, packs, grades, and references are unchanged. Re-sync from upstream rather than editing the body; `questions/`, `packs/`, `grades/`, and `references/` resolve relative to this file.
...
Treat `$scrutiny lite` or `$scrutiny quick` as an explicit Core request. Treat `$scrutiny deep`, `$scrutiny full`, and `$scrutiny audit` as explicit Full requests. Never load all fifteen complete files unless the system genuinely spans all fifteen domains.
...
A grade is different from a pack: it does not add topic depth, it sets the target the whole run is aiming for and changes what "done" means. Load one when the user names a target explicitly — "make this production ready", "MVP is fine", "get this to production grade", `$scrutiny mvp`, `$scrutiny production` — or when the request states it ("this needs to survive real traffic", "just a prototype for now").
...
Scrutiny is not an instruction to exhaust a questionnaire. Its purpose is to reduce material uncertainty until the agent has a solid, evidence-backed basis to act.
