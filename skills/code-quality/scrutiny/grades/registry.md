...
ready", "MVP is fine for now", "get this to production grade", `$scrutiny
mvp`, `$scrutiny production` — or when the request itself states the target
...
3. Gate items are self-answered the same way everything else in Scrutiny is:
